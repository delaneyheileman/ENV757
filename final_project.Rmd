...
#This r-markdown is for the purposes of analyzing the continuous variables of kilowatt-hours versus annual gross income and heat pump 
...
ancmod$transKWH <- (ancmod$KWH^(1/2))
...
```{r}
fitanco <- lm(transKWH ~ MONEYPY + CENACHP, data = ancmod)

#removed(MONEYPY*CENACHP)#

Anova(fitanco, type = 3) #error because there's an interaction that doesn't have an interaction 
summary(fitanco)

anccoef <- coef(fitanco)
plot(transKWH ~ MONEYPY, data = ancmod, col = factor(CENACHP), pch = 16, cex = .5) #why is it boxplots-- because MONEYPY is categorical, you should treat moneypy as continuous 
```
